...
import { SkillCommand, RecognizedCommand, NONE_INTENT } from './skillCommand';
...
export class ConfiguredSkillCommand extends SkillCommand {
    ...
    protected async onRecognizeCommand(context: TurnContext, recognized: RecognizerResult): Promise<RecognizedCommand|undefined> {
        const top = SkillCommand.topIntent(recognized);
        if (this.intentName && top.name === this.intentName) {
            ...
            let argv = SkillCommand.utteranceToArgv(context);
